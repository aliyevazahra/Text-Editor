...
class FindDialog(QDialog):
    ...
    def UI(self):
        ...
        hbox.addWidget(self.btn_Replace)
        formLayout.addRow(txt_Find, self.findInput)
        formLayout.addRow(txt_Replace, self.replaceInput)
        formLayout.addRow(txt_Empty,hbox)
...
class Main(QMainWindow):
    ...
    def UI(self):
        ...
        self.editor.setFontPointSize(18.0)
    ...
    def menu(self):
        ...
        ####################################################################################
        toggleStatusBar = QAction("Toggle Status Bar", self)
        ...
        ####################################################################################
        toggleToolBar = QAction("Toggle Tool Bar", self)
        toggleToolBar.setCheckable(True)
        toggleToolBar.triggered.connect(self.funcToggleToolBar)
        view.addAction(toggleToolBar)
        ####################################################################################
        toggleDockBar = QAction("Toggle Dock Bar", self)
        toggleDockBar.setCheckable(True)
        toggleDockBar.triggered.connect(self.funcDockBar)
    ...
    def toolbar(self):
        ...
        ####################################################################################
        self.fontBackColor = QAction(QIcon("icons/backcolor.png"), "Background Color", self)
        self.tb.addAction(self.fontBackColor)
        self.fontBackColor.triggered.connect(self.funcFontBackColor)
        self.tb.addSeparator()
        self.tb.addSeparator()
        ####################################################################################
        self.alignLeft = QAction(QIcon("icons/alignleft.png"), "Align text to the left.", self)
        self.tb.addAction(self.alignLeft)
        self.alignLeft.triggered.connect(self.funcAlignLeft)
        ####################################################################################
        self.alignCenter = QAction(QIcon("icons/aligncenter.png"), "Align text to the center", self)
        self.tb.addAction(self.alignCenter)
        self.alignCenter.triggered.connect(self.funcAlignCenter)
        ####################################################################################
        self.alignRight = QAction(QIcon("icons/alignright.png"), "Align text to the right.", self)
        self.tb.addAction(self.alignRight)
        self.alignRight.triggered.connect(self.funcAlignRight)
        ####################################################################################
        self.alignJustify = QAction(QIcon("icons/alignJustify.png"), "Justify text(align to the left and the right)",
                                    self)
    ...
    def funcFontBackColor(self):
        ...
        self.editor.setTextBackgroundColor(bgcolor)
    ...
    def funcTextChanged(self):
        ...
        self.status_bar.showMessage("Number of Characters: " + str(letters) + "   " + "Number of Words: " + str(words))
    ...
    def Find(self):
        ...
        def findWords():
            ...
            if word != "":
                ...
                QMessageBox.information(self, "Warning", "The field cannot be empty.")
        ...
        self.find.btn_Find.clicked.connect(findWords)
        self.find.btn_Replace.clicked.connect(replaceWords)
        ...
        try:
            if textChanged == True:
                if url !="":
                    ...
                    content_2 = self.editor.toPlainText()
                    with open(url[0], 'w', encoding='utf-8') as file_2:
    ...
    def exitFile(self):
        ...
        try:
            if textChanged == True:
                mbox = QMessageBox.information(self, "Warning", "Do you want to save this file?",
...
def main():
    ...
    App.setWindowIcon(QIcon('icons/text-edit.ico'))
    window = Main()
    sys.exit(App.exec_())
